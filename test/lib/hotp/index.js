...
describe('HOTP test case', function () {
	...
	describe('Token Generation with various token output length',  () => {
		...
		token_lengths.forEach(function (token_length) {
			describe(`Token generation with ${token_length} digits output`, () => {
				...
				beforeEach(`Generate HOTP with ascii secret ${asciiSecret}`, () => {
					let hotp = new Hotp({tokenLength: token_length});
	...
	describe('Token Validation', function () {
		describe(`With Ascii secret ${asciiSecret}`, () => {
			let hotp = null;
			let token = null;
			beforeEach('HOTP instantiation', () => {
				hotp = new Hotp();
				token = hotp.createToken({secret: asciiSecret, counter: counter});

			});
			describe('',() => {
				it('Token with counter 0 and window 0 should be valid', () => {
					const isValid = hotp.validate(
						{token: token, counter: 0,
							secret: asciiSecret, window: 0});
					assert.isTrue(isValid);
				});

				it(`Token with counter 0 and window 8 should be valid`, () => {
					const isValid = hotp.validate({
						token: expectedTokens[7][1].toString().slice(-6),
						counter: 0,
						secret: asciiSecret,
						window: 8
					});
				...
				it(`Token with counter 0 and window 4 should be invalid`, () => {
					const isValid = hotp.validate({
						token: expectedTokens[7][1].toString().slice(-6),
						counter: 0,
						secret: asciiSecret,
			...
			it('Token with bad digest should be  invalid', function () {
				expect(() => {
					hotp.createToken({secret: asciiSecret, counter: counter, encoding: 'hex', secretAlgorithm: 'md5'});
				}).to.throw(Error);
			});
			*/
	...
	describe('Token Validation', () => {
		describe(`With hexadecimal  secret ${hexSecret}`, () => {
			...
			beforeEach('TOTP instanciation', () => {
				hotp = new Hotp();
				token = hotp.createToken({secret: hexSecret, counter: counter, encoding: 'hex'});
			});

			it(`Token with counter 0 should be valid`, () => {
			...
			it(`Token with counter 0 should be invalid`, () => {
