...
const digestMap = new Map([['sha1',DigestAlgorithms.SHA1],['sha256',DigestAlgorithms.SHA256],['sha512',DigestAlgorithms.SHA512]]);
	...
	describe('Generating token', () => {
		...
		testDatas.forEach((data) => {
			it('Ascii secret ' + secret + ' and ' + data.seconds + ' second should give token ' + data.expected_token, () => {
				console.log(digestMap.get(data.algorithm))
				const totp = new Totp({tokenLength: 8, algorithm: digestMap.get(data.algorithm)});
	...
	describe('Validate token', () => {
		...
		testDatas.forEach((data) => {
			it('Ascii secret ' + secret + ' and ' + data.seconds + ' second should give token ' + data.expected_token, () => {
				...
				const token = totp.createToken({seconds: data.seconds, secret: secret,algorithm: digestMap.get(data.algorithm)});
				totp.validate({token: token, seconds: data.seconds,secret: secret,algorithm:digestMap.get(data.algorithm)});
				assert.equal(token, data.expected_token);
			});
		});
	});
